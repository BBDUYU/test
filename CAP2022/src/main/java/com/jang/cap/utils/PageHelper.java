...
public class PageHelper {
	...
	// 현재 페이지와 전체 레코드수를 입력 받아 페이징 표시 문자열을 반환한다. 
     public StringBuffer getPageUrl(SearchVO searchVO, int totalRow) {
		...
		if(currentBlock > 1){
			pageHtml.append("<span><a href=\"qna?page="+(currentPage-1)+ smsg + "\" ><이전></a>&nbsp;&nbsp;");
		}else{
		...
		for(int i = startPage ; i <= endPage ; i++) {
			if(i == currentPage){
				...
				pageHtml.append(".&nbsp;<a href=\"qna?page="+i+ smsg + "\" class=\"page\">" +i+"</a>&nbsp;");
			}		
		}
		//현재 블럭안이 전체 블럭 보다 작으면 [다음] 표시
		if(currentBlock < totalBlock){
			pageHtml.append(".&nbsp;&nbsp;<a href=\"qna?page="+(currentPage+1)+ smsg + "\"><다음></a></span>");
		}else{
			pageHtml.append(".</span>");
		}
