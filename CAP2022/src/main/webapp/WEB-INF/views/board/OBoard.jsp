...
function moveAction(where){
	...
	if(confirm("위시리스트에 추가하시겠습니까?")){
		location.href = "oboard?ono=${one.ono}";
	}
	break;
	  }
	}
  ...
  <section class="py-5 text-center container">
    <div class="row py-lg-5">
      <div class="col-lg-6 col-md-8 mx-auto">
        <h1 class="fw-light" span style="font-family: 'Gowun Dodum' !important; font-size:80px">하루 투어</h1>
        <p class="lead text-muted"span style="font-family: 'Gowun Dodum' !important; font-size:30px">하루 투어는 레저 및 체험 활동을 하루 종일 즐기고 싶은 레저 매니아 분들에게 추천합니다.</p>
    ...
    <div class="container">
      ...
      <c:forEach var="o" items="${oList}">
        ...
        <div class="col">
          ...
          <a href="oview?ono=${o.ono}" >
            <img src="<c:url value='/resources/images/${o.img1}'/>" href="oview?ono=${o.ono}" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Placeholder: Thumbnail" preserveAspectRatio="xMidYMid slice" focusable="false"><title>Placeholder</title><rect width="100%" height="100%" fill="#55595c"/><text x="50%" y="50%" fill="#eceeef" dy=".3em"></text></svg>
			</a>
            <div class="card-body">
              <p class="card-text" span style="font-family: 'Gowun Dodum' !important;font-size:18px">${o.name}</p>
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
               
        			<form name="obwish" action="obwish" method="post" onsubmit="return errCodeCheck()">
                
                	 <input type="hidden" id="ono" name="ono" value="${o.ono}" />
                	 <input type="hidden" id="userid" name="userid" value="${userId}" />
                	 <input type="hidden" id="img1" name="img1" value="${o.img1}" />
                     <input type="hidden" id="name" name="name" value="${o.name}" />
                ...
                <small class="text-muted" span style="font-family: 'Gowun Dodum' !important;font-size:18px">${o.price}</small>
              </div>
            </div>
          </div>
        </div>
      ...
      <a class="btn btn-sm btn-outline-secondary" href="/cap/board/owrite">새 상품 등록</a> 
   <% } %>
   </div>
    </div>
